 ...
 * @version $Revision: 18474 $ $Date:: 2008-01-08$
 * @since 21.03.2004
 * 
...
public abstract class MPPSManagerBean implements SessionBean {
    ...
    public List<Dataset>[] updateScheduledStepAttributes(Dataset mwlitem,
        ...
        List<Dataset>[] mppsAndStudiesToMove = new List[] {new ArrayList<Dataset>(c.size()), null};
        for (Iterator it = c.iterator(); it.hasNext();) {
            ...
            if (!pat.isIdentical(priorPat)) {
                ...
                log.info(prompt);
                mppsAndStudiesToMove[1] = updatePatientOfMppsAndStudy(suid, mpps, pat);
            }
            Dataset attrs = mpps.getAttributes();
            if (log.isDebugEnabled()) {
            ...
            Boolean mppsUpdated = updateScheduledStepAttributes(mwlitem, ssasq, discontinued);
            if (mppsUpdated != null) {
                if (mppsUpdated) {
                    mpps.setAttributes(attrs);
                ...
                mppsAndStudiesToMove[0].add(attrs);
            }
        }
        return mppsAndStudiesToMove;
    }
